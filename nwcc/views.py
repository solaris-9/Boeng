...
def nwcc_list(request):
    ...
    if level != 'undefined':
        cmd = 'SELECT {fields} FROM {tbl} '.format(
            fields=','.join(
                ['`{field}`'.format(field=f) for f in nwcc_fields.keys()]
            ),
            tbl=tbl
        )
        ...
        logger.debug(f'nwcc_list, sql = {cmd}')

        df = db.read_query(cmd)
        df = df.replace({np.nan: None}).fillna('')
        for i_index in df.index:
            item = {}
            for field in nwcc_fields.keys():
                match nwcc_fields[field]['type']:
                    case 'str':
...
def handle_nwcc_edit(tbl, data):
    generated_str = u.generate_update_sql(nwcc_fields, data, ['creator', 'createon'])
    ...
    logger.debug(f'handle_nwcc_edit, sql = {sql}')
    db.execute(sql)
...
def handle_nwcc_delete(tbl, llist, mmail):
    sql = 'delete from {tbl} where `ID` in ({B_LIST})'.format(
        tbl=tbl,
        B_LIST=u.generate_delete_sql(llist)
    )
    logger.debug(f'handle_nwcc_delete, sql = {sql}')
    db.execute(sql)
...
def handle_nwcc_add(tbl, data):
    ...
    generated_str = u.generate_insert_sql(nwcc_fields, l_data, skip=['modifier', 'modifiedon'])
    ...
    logger.debug(f'handle_nwcc_add: sql = {sql}')
    db.execute(sql)
...
def nwcc_edit(request):
    logger.debug('nwcc_edit, request.body:', request.body.decode('utf-8'))
    res = {
        'code': 20000,
        'data': {
            'items': [],
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if data:
                ...
                if ttype in ['add', 'edit']:
                    l_data = {}
                    for field in [f for f in nwcc_fields.keys() if f != 'ID']:
        ...
        logger.debug(f'nwcc_edit, Invalid Parameters: {e}')
        res['data']['status'] = "Invalid Parameters"
        return HttpResponse(simplejson.dumps(res), content_type='application/json')

    # 1x add
    if ttype == 'add':
        rt = handle_nwcc_add(tbl, l_data)
        res['data']['status'] = rt
            
    # 2 edit
    elif ttype == 'edit':
        handle_nwcc_edit(tbl, l_data)
        res['data']['status'] = "Edit successful"
        pass

    # 3 delete
    elif ttype == 'delete':
        handle_nwcc_delete(tbl, l_delete_list, mail)
        pass
